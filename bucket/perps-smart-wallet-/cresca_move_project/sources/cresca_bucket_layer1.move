// Move module for Cresca Layer1 Bucket Protocol - MULTI-USER VERSION
// Bucket 3: Layer 1 Assets - ADA, AVAX, DOT
module cresca::bucket_layer1 {
    use std::vector;
    use std::signer;
    ...
    /// Hardcoded token addresses - Layer 1 assets
    const ADA_ADDRESS: address = @0x4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5;
    const AVAX_ADDRESS: address = @0x5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6;
    const DOT_ADDRESS: address = @0x6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7;

    /// Hardcoded weights: ADA 30%, AVAX 40%, DOT 30%
    const ADA_WEIGHT: u64 = 30;
    const AVAX_WEIGHT: u64 = 40;
    const DOT_WEIGHT: u64 = 30;
    ...
    fun create_default_bucket_internal(_owner: &signer, leverage: u64) acquires Protocol {
        ...
        vector::push_back(&mut assets, ADA_ADDRESS);
        vector::push_back(&mut assets, AVAX_ADDRESS);
        vector::push_back(&mut assets, DOT_ADDRESS);
        
        let weights = vector::empty<u64>();
        vector::push_back(&mut weights, ADA_WEIGHT);
        vector::push_back(&mut weights, AVAX_WEIGHT);
        vector::push_back(&mut weights, DOT_WEIGHT);
    ...
    public entry fun update_oracle(
        ...
        vector::push_back(&mut prices, ada_price);
        vector::push_back(&mut prices, avax_price);
        vector::push_back(&mut prices, dot_price);
    ...
    fun calculate_weighted_price(protocol: &Protocol): u64 {
        ...
        let ada_price = *vector::borrow(prices, 0);
        let avax_price = *vector::borrow(prices, 1);
        let dot_price = *vector::borrow(prices, 2);
        
        let weighted = (ada_price * ADA_WEIGHT + avax_price * AVAX_WEIGHT + dot_price * DOT_WEIGHT) / 100;
        weighted
    }
