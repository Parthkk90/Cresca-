...
# Run: .\verify-contract-simple.ps1
...
function Test-ContractExists {
    ...
    try {
        ...
        if ($response -and $response.Count -gt 0) {
            ...
            # Check for bucket_protocol specifically
    ...
    try {
        ...
        if ($response) {
            ...
            if ($protocolResource) {
                ...
                Log-Test "Protocol resource" "INFO" "Protocol not initialized yet (run init() first)"
                return $false
            }
        } else {
...
function Test-ViewFunction {
    ...
    try {
        ...
        if ($_.Exception.Message -like "*RESOURCE_NOT_FOUND*") {
            Log-Test "$FunctionName view" "INFO" "Resource not found (init() not called yet)"
        } else {
...
function Test-AllViewFunctions {
    ...
    # get_position_details needs position_id, skip if no positions
    # $results["get_position_details"] = Test-ViewFunction "get_position_details" @($CONTRACT_ADDRESS, "0")
...
function Show-NextSteps {
    ...
    Write-Host "  1. Download: https://nodejs.org/" -ForegroundColor White
    ...
    Write-Host "  1. Download: https://aptos.dev/tools/aptos-cli/install-cli/" -ForegroundColor White
...
function Main {
    ...
    Write-Host "║     BUCKET PROTOCOL - SIMPLE VERIFICATION (NO INSTALL)        ║" -ForegroundColor Cyan
    Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
